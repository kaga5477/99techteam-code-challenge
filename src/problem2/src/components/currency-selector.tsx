...
export const CurrencySelector: React.FC<CurrencySelectorProps> = ({
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button
        type="button"
        className="cursor-pointer flex items-center justify-between min-w-full px-4 py-2 text-lg font-semibold text-gray-800 bg-white border border-gray-300 rounded-lg shadow-sm hover:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-200 ease-in-out"
        onClick={() => setIsOpen(!isOpen)}
        aria-haspopup="listbox"
        aria-expanded={isOpen}
      >
        <span className="flex items-center">{selectedCurrency || label}</span>
        <ChevronDown
